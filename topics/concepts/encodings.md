...
data stored in the computer's memory (which we can think of as an `Array[Byte]`), so while the `Char`
abstraction hides this detail from us, some translation between characters and their binary representations is
nevertheless happening whenever strings are placed in or retrieved from memory.
...
While many dozens of different character encodings exist, it is useful to know about three that are currently in
widespread usage today: `ASCII`, `UTF-8` and, primarily in the English-speaking world, `ISO-8859-1`.
...
replaced by their closest equivalent letters from the 26-letter English alphabet.
...
character, with a single bit spare, and this is how ASCII text is normally transmitted, though forms of
compression exist which avoid wasting one bit for every character, and can store ASCII in ⅞ of the space it
would normally take.
...
additional letters which, when combined with the English letters defined in ASCII, support about thirty
different recognised world languages which generally use the Latin alphabet (generally the western European
languages).

In fact, a series of less widely-used character encodings exist, numbered `ISO-8859-x` for `x` up to `16`, each
combining the ASCII character encoding with additional characters to support other languages.
...
different results when run on different machines, if those machines have different character encodings.
...
`ISO-8859-1`, and none could be misinterpreted.

That worked adequately for computers which generally consumed only text which they had themselves produced, but
since the Internet was created, it became much more common for text to be exchanged between computers with
different system encodings.

It is therefore usually preferable to be explicit about the character encoding whenever a conversion is
necessary.
...
sequences in each character encoding.
...
Furthermore, every byte of a `UTF-8` encoding of a Unicode character which requires two or more bytes will _not_
be a valid ASCII character, which means that every byte in the sequence will be interpreted as a negative
number.

We can be more explicit about the encoding by specifying the character encoding, as a string itself, as a
parameter to the `String#getBytes` method, for example when calling, `"Café".getBytes("UTF-8")`.
...
This may seem surprising, since the letter `é`, like many other accented letters which rarely appear in English
words, cannot be encoded with ASCII! But we can inspect the result when decoding.
...
five given bytes using the system encoding (which we know will work, since all are in the common ASCII subset).

But for an array of bytes originating elsewhere, called `bytes` for example, we should specify the character
encoding explicitly, for example with, `String(bytes, "UTF-8")`.

## Missing Characters

If we consider the example of encoding `"Café"` using ASCII, we earlier obtained the byte array
...
compromise and used a substitute character, `?`, in place of `é`. Sometimes it would be preferable to know that
it was not possible te encode a character precisely, but the `getBytes` method always returns a result and will
silently replace any characters it cannot encode with characters it can.
...
The most common mismatch involves both `UTF-8` and `ISO-8859-1`, and we can simulate these mismatches with a
couple of examples:
...
Since these incorrect encodings both appear frequently, it's useful to learn to identify them, as it can be the
first step towards identifying the source of the problem.

Of course, the spurious characters (or missing characters) are not the same each time, so the best indication is
the number of characters that appear, since `ISO-8859-1` always assumes a single byte per character, while
`UTF-8` may use more than one byte for one character.

So if the text appears to contain two or more unexpected characters where a single non-ASCII character would be
expected, then this is likely to represent `UTF-8`-encoded text being interpreted as `ISO-8859-1`, while missing
characters or fewer characters than expected being displayed probably represents the reverse.
...
And while Unicode may one day become the only encoding in use, its predecessors are likely to have a long
legacy. So for as long as that remains the case, we need to be aware that text may be stored using different
character encodings, and cognizant of the challenges of working in a multi-encoding world.

?---?

# A string encodes to the byte array, `Array(-62, -91, 49, 48, 48)`, but we do not know what encoding was used.

Without knowing any more about the content of the string, which of the following encodings might have been used?
...
# Another string encodes to the byte array, `Array(-27, 110, 103, 115, 116, 114, -10, 109)`.

Again, without knowing the content of the string, or its encoding, which of the following encodings might have
been used?
...
# The string, `El NiÃ±o`, appears on a web page. Which of the following is the most likely explanation for this?

- [ ] The web page was encoded with `ISO-8859-1` and is being interpreted as `UTF-8`
- [X] The web page was encoded with `UTF-8` and is being interpreted as `ISO-8859-1`
- [ ] The author of the page intended for `El NiÃ±o` to be displayed

# The string, `Gda?sk Café`, appears on a web page, where you would expect to see `Gdańsk Café`.

Which of the following are probable explanations?

* [ ] The web page was encoded and decoded with `UTF-8`
* [X] The web page was encoded and decoded with `ISO-8859-1`
* [ ] The web page was encoded and decoded with `ASCII`
